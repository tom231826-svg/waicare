...
def test_active_provider_autodetect(monkeypatch):
    monkeypatch.delenv("WAICARE_LLM_PROVIDER", raising=False)
...
def test_unknown_provider_raises(monkeypatch):
    monkeypatch.setenv("WAICARE_LLM_PROVIDER", "wizard")
...
def test_compose_falls_back_to_template_without_llm(monkeypatch, fiji_config, playbooks_dir):
    ...
    # No backend → falls back to the static English template, delivery still happens.
